...
@health_bp.route('/thank-you')
def thank_you():
    return render_template('thank_you.html')


@health_bp.route('/privacy')
def privacy():
    return render_template('privacy.html')


@health_bp.route('/terms')
def terms():
    return render_template('terms.html')


@health_bp.route('/about')
def about():
    return render_template('about.html')


@health_bp.route('/robots.txt')
def robots():
    sitemap_url = f"{current_app.config['PUBLIC_BASE_URL']}/sitemap.xml" if current_app.config['PUBLIC_BASE_URL'] else url_for('health.sitemap', _external=True)
    body = "User-agent: *\nAllow: /\nDisallow: /dashboard\nDisallow: /history\nDisallow: /records\nDisallow: /watch\nDisallow: /chatbot\nDisallow: /diabetes\nDisallow: /heart\nDisallow: /brain-tumor\nDisallow: /api/\nDisallow: /thank-you\nSitemap: " + sitemap_url + "\n"
    return Response(body, mimetype='text/plain')


@health_bp.route('/sitemap.xml')
def sitemap():
    public_endpoints = ['health.index', 'health.about', 'health.privacy', 'health.terms']
    base_url = current_app.config['PUBLIC_BASE_URL']
    urls = '\n'.join(
        f'  <url><loc>{base_url}{url_for(endpoint) if base_url else url_for(endpoint, _external=True)}</loc></url>'
...
def analyze():
    ...
    try:
        ...
        return jsonify({'error': 'Please review your health inputs and enter valid numeric values.'}), 400
